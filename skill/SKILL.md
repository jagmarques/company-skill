...
**company-context-guard**: blocks when token fill reaches the configured threshold (default 50%). Reads the transcript for the latest assistant message, sums `input_tokens + cache_read_input_tokens + cache_creation_input_tokens`, and divides by the detected context window. Window detection: reads the model id from the transcript - 1M for Opus 4 and any model whose id contains `[1m]`, 200K otherwise. Unknown models default to 1M (fail-open, a false block would brick the session). `COMPANY_CONTEXT_WINDOW` overrides the detection. `COMPANY_CONTEXT_THRESHOLD` sets the threshold (fraction like `0.4` or percent like `40`). The block reason names the fill percentage and instructs the operator to run /company restart. This guard fails open on any parse or read error - it is a productivity guard, not a security gate. Enforcement is the hard block plus the forcing instruction (no programmatic /compact). **Per-session toggle**: the hard block can be disabled for a specific session by setting `enforceRestart: false` in `.company/context-guard-config.json` under `sessions["<sessionId>"]`. The dashboard "Auto-restart at N%: ON/OFF" button near the context gauge writes this config automatically. The default (no file, no entry, malformed file) is `enforceRestart: true` so existing behavior is unchanged. A malformed config file always falls back to enforce=true (fail-safe).
